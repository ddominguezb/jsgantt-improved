...
import {
  ...
  addListenerResizeList,
  addListenerResizeGant,
...
export const GanttChart = function (pDiv, pFormat) {
  ...
  this.calculateHeightContainers = function () {
    ...
    this.vListContainer.style.minHeight  = (((this.vNumRows + 1) * this.vRowHeight) + 2) + 'px';
    this.vListContainer.style.maxHeight = (((this.vNumRows + 1) * this.vRowHeight) + 2) + 'px';
    ...
    this.vChartContainer.style.minHeight = (((this.vNumRows + 1) * this.vRowHeight) + 2) + 'px';
    this.vChartContainer.style.maxHeight = (((this.vNumRows + 1) * this.vRowHeight) + 2) + 'px';
  ...
  this.drawListHead = function (vLeftHeader) {
    ...
    this.vListHead = newNode(vTmpDiv, 'div', null, 'gvs-tablehader');
    let vTmpRow = newNode(this.vListHead, 'div', null, 'gvs-row');
    ...
    vTmpRow = newNode(this.vListHead, 'div', null, 'gvs-row');
  ...
  this.drawListBody = function (vLeftHeader, gListLbl) {
    ...
    let vNumRows = this.vNumRows;

    syncScroll([vTmpContentTabOuterWrapper, gListLbl], 'scrollLeft');
    //vNumRows = this.updateListContainer();
  ...
  this.updateListContainer = function () {
    ...
    for (j = startRow; j < visibleRowsCount + startRow; j++) {
      ...
      vTmpCell.title = this.vTaskList[i].getName();
  ...
  this.drawChartHead = function (vMinDate, vMaxDate, vColWidth, vNumRows) {
    ...
    while (vTmpDate.getTime() <= vMaxDate.getTime()) {
      ...
      if (this.vFormat == 'day') {
        ...
        let vTmpCell = newNode(vTmpRow, 'div', null, vHeaderCellClass, null, vColWidth * colspan, null, null, colspan);
        ...
        const vTmpCell = newNode(vTmpRow, 'div', null, vHeaderCellClass, null, vColWidth);
        ...
        const vTmpCell = newNode(vTmpRow, 'div', null, vHeaderCellClass, null, vColWidth * vColSpan, null, null, vColSpan);
    ...
    vTmpRow = newNode(vTmpTBody, 'div', null, 'footerdays gvs-row');
    ...
    while (vTmpDate.getTime() <= vMaxDate.getTime()) {
      ...
      if (this.vFormat == 'day') {
        ...
        if (vTmpDate <= vMaxDate) {
          const vTmpCell = newNode(vTmpRow, 'div', null, vMinorHeaderCellClass, null, vColWidth);
        ...
        if (vTmpDate <= vMaxDate) {
          const vTmpCell = newNode(vTmpRow, 'div', null, vMinorHeaderCellClass, null, vColWidth);
          newNode(vTmpCell, 'div', null, null, formatDateStr(vTmpDate, this.vWeekMinorDateDisplayFormat, this.vLangs[this.vLang]));
          vNumCols++;
        }
        ...
        if (vTmpDate <= vMaxDate) {
          const vTmpCell = newNode(vTmpRow, 'div', null, vMinorHeaderCellClass, null, vColWidth);
          newNode(vTmpCell, 'div', null, null, formatDateStr(vTmpDate, this.vMonthMinorDateDisplayFormat, this.vLangs[this.vLang]));
          vNumCols++;
        }
        ...
        if (vTmpDate <= vMaxDate) {
          const vTmpCell = newNode(vTmpRow, 'div', null, vMinorHeaderCellClass, null, vColWidth);
          newNode(vTmpCell, 'div', null, null, formatDateStr(vTmpDate, this.vQuarterMinorDateDisplayFormat, this.vLangs[this.vLang]));
          vNumCols++;
        }
        ...
        for (let i = vTmpDate.getHours(); i < 24; i++) {
          ...
          if (vTmpDate <= vMaxDate) {
            const vTmpCell = newNode(vTmpRow, 'div', null, vMinorHeaderCellClass, null, vColWidth);
            newNode(vTmpCell, 'div', null, null, formatDateStr(vTmpDate, this.vHourMinorDateDisplayFormat, this.vLangs[this.vLang]));
            vNumCols++;
          }
        }
        vTmpDate.setHours(0);
  ...
  this.drawCharBody = function (vTmpContentTabWrapper, gChartLbl, gListLbl,
    ...
    syncScroll([this.vListBody, vTmpDiv], 'scrollTop');
    ...
    //addListenerResizeGant(this,vTmpDiv);
  ...
  this.drawCharGrid = function () {
    if (!this.vSingleCell) {
      ...
      for (i = startRow; i < visibleRowsCount + startRow; i++) {
        left = startCol * this.vColWidth;
        for (j = startCol; j < visibleColsCount + startCol; j++) {
  ...
  this.drawCharTaskBars = function () {
    ...
    this.setFirstRowIndex( this.vVisibleRowsIdxItems[startRow] );
    this.setLastRowIndex( this.vVisibleRowsIdxItems[(visibleRowsCount + startRow)-1] );

    /** 
     * NOTE: The first row index and last row index are the index of row items (task with a visual row in the table)
     *       We can have another items in Task list between those index
    **/ 
    for (i = this.getFirstRowIndex() ; i <= this.getLastRowIndex() ; i++) {
      if (this.vTaskList[i].getVisible() != 0) {
  ...
  this.updateCharBody = function () {
    this.drawCharBackgroundRows(this.vNumCols * this.vColWidth);
    this.drawCharGrid();
  ...
  this.Draw = function () {
    ...
    */
    const { vNumRows,
      vTmpContentTabWrapper } = this.drawListBody(vLeftHeader, gListLbl);
    ...
    addListenerInitScenario(this, this.vChartBody);
    ...
    vTmpDiv.style.height = this.vTotalHeight + 'px';
    ...
    addVirtualScrollListener(this, this.vChartBody);
    ...
    this.vListContainer.style.width = this.vListHead.offsetWidth + 'px';
